...
#ifdef CONFIG_WAPI
/* These are vendor-specific hacks to linux/wireless.h */
/*
...
static void wext_get_channel(struct iw_event *iwe, u32 *channel)
{
	...
	if (iwe->u.freq.e > 6) {
		wpa_printf(MSG_DEBUG, "WEXT: Invalid freq m=%d e=%d)",
					   ...
					   char *data, int len)
{
	...
	while (pos + IW_EV_LCP_LEN <= end) {
		...
		case SIOCGIWFREQ:
		{
			...
			wext_get_channel(iwe, &ev.channel.channel);
			wpa_printf(MSG_INFO, "Wireless event: Channel %d",
				   ev.channel.channel);
			wpa_supplicant_event(drv->ctx, EVENT_CHANNEL, &ev);
			break;
		}
		case SIOCGIWAP:
				 ...
				 char *end)
{
	...
	if (clen > 7 && os_strncmp(custom, "wpa_ie=", 7) == 0) {
		...
		/* u8 bin[8]; */
		int bytes;
		wpa_printf(MSG_DEBUG, "[get wapi ie]\n"); /* Dm: */
		spos = custom + 8;
		bytes = custom + clen - spos;
		if (bytes & 1)
			return;
...
#define SSID_MAX_WAPI_IE_LEN 100
		if (bytes > SSID_MAX_WAPI_IE_LEN) {
			wpa_printf(MSG_INFO, "Too long WAPI IE (%d)", bytes);
			return;
		}
...
#ifdef CONFIG_WAPI
	/*
	 * FIX: What on earth is this trying to do? Cause an infinite loop if
	 * the driver doesn't support SIOCGIWRANGE?!
	 */
	while (1) {
...
#ifdef CONFIG_WAPI
	/* Clear APP IE */
	wpa_driver_wext_set_gen_ie(drv, (const u8 *) "", 0);
#endif /* CONFIG_WAPI */
